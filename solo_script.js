...
for(var i = 0; i < array.length; i++){
	array[i] = calculateSTI(array[i]); // Was passing the entire table through, needed to change to just pass through rows
...
function calculateSTI(array){
  ...
  if(bonus > 0.13){
    bonus = 0.13;
		// add else if to set perameter for minimum of 0
  } else if (bonus < 0){
    bonus = 0;
  }
  ...
  console.log(newArray[0] + " " + newArray[1] + " " + newArray[2] + " " + Math.round(newArray[3]));
  return newArray;
}
...
function getBaseSTI(reviewScore){
  ...
  return basePercent; //removed the -1 here
}
